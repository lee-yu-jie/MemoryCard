...
let originalImgList: stringArray = [
  'Angular',
  'Pinia',
  'React',
  'Vue',
...
const randomImgList: stringArray = []
const cardArr: cardInfo[] = []
const cardAmount = 24
let amountOfSuccessfulMatches = 0
let timer: number
let matchingCardArr: HTMLElement[] = []
let isStart = false
...
cardArr.forEach(cardInfo => {
  ...
  card.addEventListener('click', function(this: HTMLElement){
    if(matchingCardArr.length === 2) return
    ...
    if (this.classList.contains('active')) {
      ...
      matchingCardArr = matchingCardArr.filter((card: HTMLElement) => card.dataset.id !== this.dataset.id);
    }
...
function createCard (cardInfo: cardInfo){
...
function formatTime(){
  let time = 0
  const timeArea = document.querySelector('.time-area') as HTMLElement;
  timer = setInterval(() => {
    time++
...
function handleCompareCards (areTwoCardsTheSame: boolean){
...
function handelFinishGame(isFinish: boolean){
  if(isFinish){
    clearInterval(timer)
    setTimeout(() => {
      alert('恭喜你完成遊戲')
